...
**What it is:** A documented example of the most common unit of work in your repo. When the agent needs to "add a new X", this is the blueprint. It's the document you'd hand a new teammate on day one: "here's how we do things, follow this shape."
...
5. Note the WHY for non-obvious decisions — this is what separates a pattern doc from a code snippet

**What a good pattern doc includes:**
- The files to create (in order)
- Complete code for each file (not pseudocode — real, copy-pasteable code)
- The test cases to cover (happy path, validation, error paths)
- A "Why this structure?" section explaining non-obvious decisions

→ **[See a complete example: API endpoint pattern](examples/reference-pattern.md)**
